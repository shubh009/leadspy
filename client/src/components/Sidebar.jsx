      ...
      <div className="space-y-4 pt-4">
        ...
        {currentUser && (
          <div className="flex items-center gap-2.5 px-2 py-1.5 rounded-xl bg-white/5 border border-white/5">
            <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-amber-500 to-orange-500 flex items-center justify-center text-white font-bold text-xs shrink-0 shadow-inner">
              {currentUser.avatar ? (
                <img src={currentUser.avatar} alt={currentUser.name} className="w-full h-full rounded-full object-cover" />
              ) : (
                <span>{(currentUser.name || currentUser.email || 'A')[0].toUpperCase()}</span>
              )}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold text-white truncate">{currentUser.name || 'Agent LeadSpy'}</p>
              <p className="text-[10px] text-gray-400 truncate">{currentUser.email}</p>
            </div>
            <span className="w-2 h-2 rounded-full bg-emerald-500 shrink-0" title="Online & Connected" />
          </div>
        )}
        ...
        <button 
          onClick={onLogout}
          className="w-full flex items-center justify-between px-3.5 py-2 text-xs font-medium text-gray-400 hover:text-rose-400 hover:bg-white/5 rounded-xl transition cursor-pointer"
        >
